...
class Common extends Model
{
    ...
    public static function ImageUpload($image,$folder) {
        ...
        $front = '../2016.mexicuga.com/public/images/'.$folder;
        $admin = public_path('images/'.$folder);
        
        if(!is_dir($front)){ mkdir($front, 0777, true); }
        $frontpath = $front.'/'.$filename;
        
        if(!is_dir($admin)){ mkdir($admin, 0777, true); }
        $adminpath = $admin.'/'.$filename;
        
        $image->save($frontpath);
        $image->save($adminpath);
