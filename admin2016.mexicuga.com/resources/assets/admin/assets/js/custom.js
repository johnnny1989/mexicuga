...
function SaveImage(form,msg,pageurl){
    ...
    $.ajax({
                ...
                success: function(data){
                 ...
                 //console.log(data);// A function to be called if request succeeds
                $('#imgloader').remove();
                $('.error').remove();
...
$('#BANNERLONUEVO').submit(function(){
    SaveImage(this,'BANNERLONUEVOform',false);
    return false;
});
...
function ChangeOrder(id,direction,path,trid){
...
function moveDown(trID) {
    ...
    document.getElementById("demo-custom-toolbar").tBodies[0].insertBefore(clone, nextElement);
    document.getElementById("demo-custom-toolbar").tBodies[0].removeChild(row);
}

function moveUp(trID) {
    var row = document.getElementById(trID);
    var clone = row.cloneNode(true);
    var currentRow = document.getElementById("demo-custom-toolbar").tBodies[0].firstChild;
    ...
    while (currentRow != row) {
        ...
        while (currentRow != null && currentRow.nodeName != "TR")
            currentRow = currentRow.nextSibling;
    }

    document.getElementById("demo-custom-toolbar").tBodies[0].insertBefore(clone, lastRow);
    document.getElementById("demo-custom-toolbar").tBodies[0].removeChild(row);
}
